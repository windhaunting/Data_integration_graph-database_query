...
import net.sf.javaml.core.SparseInstanceCustomFBW;
 ...
 * 
 */
final public class DatasetToolsCustomFBW {
	 ...
	 * 
	 */
	public static void merge(DatasetCustomFBW... datasets) {
		DatasetCustomFBW out = null;
		for (DatasetCustomFBW data : datasets) {
			if (out == null)
				out = data;
			else
				out.addAll(data);
	 ...
	 */
	@Deprecated
	public static DatasetCustomFBW bootstrap(DatasetCustomFBW data, int size, Random rg) {
	 ...
	 */
	public static InstanceCustomFBW maxAttributes(DatasetCustomFBW data) {
		InstanceCustomFBW max = new SparseInstanceCustomFBW();
		for (InstanceCustomFBW i : data) {
			for (Integer index : i.keySet()) {
				String val = i.value(index);
				if (!max.containsKey(index))
					max.put(index, val);
				else if (max.get(index).compareTo(val) < 0)            //double check here
					max.put(index, val);
	 ...
	 */
	public static InstanceCustomFBW average(DatasetCustomFBW data) {               //need to define the avearage
		/*
